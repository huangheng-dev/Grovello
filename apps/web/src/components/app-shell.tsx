...
import { getPrimaryNavigationItems, navigationSections } from '@grovello/product-config'
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const routeState = useMemo(() => {
    ...
    const item = itemSlug ? section?.items.find((candidate) => candidate.slug === itemSlug) : undefined
    return { section, item }
  }, [pathname])
  ...
  function toggleGroup(key: string) {
    const next = { ...openGroups, [key]: !(openGroups[key] ?? routeState.section?.key === key) }
    setOpenGroups(next)
      ...
      <aside className={`sidebar ${mobileOpen ? 'sidebar--mobile-open' : ''}`}>
        ...
        <nav className="sidebar-nav" aria-label={t('common.primaryNavigation')}>
          {navigationSections.map((section) => {
            const items = getPrimaryNavigationItems(section)
            const open = openGroups[section.key] ?? routeState.section?.key === section.key
            return (
              <section className="nav-section" key={section.key}>
                <div className={`nav-section__row ${routeState.section?.key === section.key ? 'nav-section__row--active' : ''}`}>
                  <Link className="nav-section__header" href={`/${locale}/${section.slug}`} title={collapsed ? t(`sections.${section.key}`) : undefined} onClick={() => setMobileOpen(false)}>
                    <Icon name={section.icon} size={19} />
                    {!collapsed ? <span>{t(`sections.${section.key}`)}</span> : null}
                  </Link>
                  {!collapsed && items.length > 0 ? <button className="nav-section__toggle" aria-label={t(open ? 'common.collapseSection' : 'common.expandSection', { section: t(`sections.${section.key}`) })} aria-expanded={open} onClick={() => toggleGroup(section.key)}><Icon name={open ? 'expand_less' : 'expand_more'} size={18} /></button> : null}
                </div>
                {open && !collapsed && items.length > 0 ? (
      ...
      <div className="workspace-shell">
        <header className="topbar">
          <div className="topbar__left">
            ...
            <div className="page-identity"><span>{t(`sections.${routeState.section?.key ?? 'command'}`)}</span><strong>{routeState.item ? t(`pages.${routeState.item.key}.title`) : t('capabilities.overviewTitle')}</strong></div>
          </div>
          <div className="topbar__right">
            ...
            <Link className="icon-button topbar-action" aria-label={t('common.help')} href={`/${locale}/command/architecture`}><Icon name="help" /></Link>
