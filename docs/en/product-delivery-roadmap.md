...
The approved P2-E boundary is specified in the [Enterprise Knowledge Ingestion and Grounded Retrieval implementation baseline](knowledge-ingestion-retrieval-baseline.md). P2-E1 now provides the locally verified contract foundation: forced-RLS execution records, narrow permissions, idempotent pending-ingestion APIs, exact immutable source snapshots, and internal-only canonical `KnowledgeChunk` creation with audit/outbox lineage. P2-E2 through P2-E4 remain pending; no parsing, embedding, lexical/semantic retrieval, citation search, or operator-UI capability is claimed, and no primary-navigation item is added.
